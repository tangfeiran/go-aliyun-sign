...
func SignCMS(req *http.Request, appKey, appSecret string) error {
	...
	req.Header.Set(HTTPHeaderCMSApiVsrsion, "1.0")
	...
	req.Header.Set(HTTPHeaderCMSSignature, "hmac-sha1")
	req.Header.Set(HTTPHeaderCMSIP, "1.1.1.1")

	req.Header.Set(HTTPHeaderAuthorization, appKey+":"+hash)
	req.Header.Set(HTTPHeaderDate, time.Now().UTC().Format(time.RFC1123))
...
func buildHeaderStringToSign(hdr http.Header) (string, []string, error) {
	...
	for _, kv := range sorted {
		...
		if strings.HasPrefix(http.CanonicalHeaderKey(kv.key), HTTPHeaderCMSPrefix) {
